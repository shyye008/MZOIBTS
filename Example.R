...
n<-60
t<-1:n;inter<-(n+1)/2
X1<-cbind(rep(1,n));X2<-X1;
X3<-cbind(rep(1,n),log(t),ifelse(t>inter,1,0),ifelse(t>inter,1,0)*(log(t)-log(inter)))
X4<-cbind(rep(1,n),ifelse(t>inter,1,0))
beta1<-log(0.95/0.05);beta2<-log(0.1/0.9)
beta3<-c(log(0.7/0.3),-0.01,0.5,0.5);beta4<-c(3,0.5)
y<-rMZOIBTS(n,"Gaussian",0.5,X1,beta1,X2,beta2,X3,beta3,X4,beta4)

# Change point selection
inter_all<-25:37
sele<-selection(X1,X2,t,y,inter_all)
inter<-sele[[1]]

# Proposed estimator
f0<-twostep.MZOIBTS(100,X1,X2,X3,X4,y,"Gaussian",0.05)
f1<-HAC.MZOIBTS(X1,X2,X3,X4,y,0.05)

# Bayesian estimator
install.packages("cmdstanr", repos = c('https://stan-dev.r-universe.dev', getOption("repos")))
library(cmdstanr)
set.seed(123)
post <- ifelse(t > inter, 1, 0)
t_cs <- (t - inter)
t_cs <- as.numeric(scale(t_cs, center = TRUE, scale = TRUE)) 

X3 <- cbind(
  rep(1, n),
  t_cs,
  as.numeric(t > inter),
  as.numeric(t > inter) * t_cs
)
...
mod <- cmdstan_model("mzoib_copula.stan")
fit_mcmc <- mod$sample(
  data = sdata,
...
fit <- fit_mcmc  
ll_draws <- fit_mcmc$draws("log_lik")
log_lik_arr <- unclass(as_draws_array(ll_draws))  
dim(log_lik_arr)  
...
log_lik_mat <- as_draws_matrix(ll_draws)   # draws x variables
keep <- grep("^log_lik\\[", colnames(log_lik_mat))
log_lik_mat <- log_lik_mat[, keep, drop = FALSE]

# 3) LOO / WAIC
loo_res  <- loo::loo(log_lik_mat, r_eff = r_eff)
waic_res <- loo::waic(log_lik_mat)
...
fit_r01=fit_mcmc
fit_mcmc$time()
fit_map <- try(mod$optimize(data = sdata), silent = TRUE)
library(posterior)
library(dplyr)
vars <- colnames(as_draws_df(fit_mcmc$draws()))
vars[grepl("beta|rho", vars)]
keep_pars <- c(
  "beta1[1]",
  "beta2[1]",
...
summ_mcmc <- fit_mcmc$summary(variables = keep_pars)
bayes_tbl <- summ_mcmc %>%
  transmute(param = variable, method = "Bayes_MCMC", est = mean, se = sd, lcl = q5, ucl = q95)

map_tbl <- NULL
if (!inherits(fit_map, "try-error")) {
  map_df  <- as.data.frame(fit_map$draws())
  keep2   <- intersect(keep_pars, colnames(map_df))
  map_tbl <- data.frame(
    param = keep2,
    method = "Bayes_MAP",
    est = as.numeric(map_df[1, keep2, drop = TRUE]),
    se  = NA_real_, lcl = NA_real_, ucl = NA_real_
  )
}

summ_all <- fit_mcmc$summary()
head(summ_all)

keep <- c("beta1[1]", "beta2[1]", "beta3[1]", "beta3[2]", "beta3[3]", "beta3[4]",
          "beta4[1]", "beta4[2]", "rho", "rho_raw")
summ_keep <- fit_mcmc$summary(variables = keep)
print(summ_keep, n = nrow(summ_keep))
...
f0<-twostep.MZOIBTS(100,X1,X2,X3,X4,y,"Gaussian",0.05)
f1<-HAC.MZOIBTS(X1,X2,X3,X4,y,0.05)
f1.2<-HAC.MZOIBTS(X1,X2,X3,X4,y,0.05,L=5)

clean_freq_tab <- function(tab) {
  rn <- make.unique(rownames(tab), sep = ".")
  rn[rn == "phi"] <- "phi1"
  idx <- grepl("^phi\\.[0-9]+$", rn)
  if (any(idx)) {
    k <- as.integer(sub("^phi\\.", "", rn[idx]))
    rn[idx] <- paste0("phi", k + 1)
  }
  rownames(tab) <- rn
  tab
}

freq0 <- clean_freq_tab(f0[[1]])
freq1 <- clean_freq_tab(f1[[1]])
freq12 <- clean_freq_tab(f1.2[[1]])

map_to_param <- c(
  "b1" = "beta1[1]",
  "b2" = "beta2[1]",
  "initial.level" = "beta3[1]",
  "initial.trend" = "beta3[2]",
  "level.change"  = "beta3[3]",
  "trend.change"  = "beta3[4]",
  "phi1" = "beta4[1]",
  "phi2" = "beta4[2]"
)

freq_long <- function(tab, label) {
  df <- data.frame(param_raw = rownames(tab), tab, check.names = FALSE)
  df <- df %>%
    mutate(param = unname(map_to_param[param_raw])) %>%
    filter(!is.na(param)) %>%    
    transmute(
      param,
      method = label,
      est = est, se = SE, lcl = CI_lower, ucl = CI_upper
    )
  df
}

freq0_tbl  <- freq_long(freq0,  "Freq_bootstrap")
freq1_tbl  <- freq_long(freq1,  "Freq_HAC_L=10")
freq12_tbl <- freq_long(freq12, "Freq_HAC_L=5")

all_tbl <- bind_rows(
  freq0_tbl, freq1_tbl, freq12_tbl,
  bayes_tbl,
  map_tbl
) %>%
  arrange(match(param, keep_pars), method)
